// $Id: SHTRS.h,v 1.3 2013/02/12 18:56:14 david Exp $
#ifndef __PF2ASM_NODE_bitwise_SHTRS_H__
#define __PF2ASM_NODE_bitwise_SHTRS_H__
...
namespace pf2asm {
  namespace node {
    namespace bitwise {
      ...
      class SHTRS: public cdk::node::Node {
      public:
        inline SHTRS(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "SHTRS"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processSHTRS(this, level);
        }
      };

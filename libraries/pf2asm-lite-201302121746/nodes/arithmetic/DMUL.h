// $Id: DMUL.h,v 1.3 2013/02/12 18:56:13 david Exp $
#ifndef __PF2ASM_NODE_arithmetic_DMUL_H__
#define __PF2ASM_NODE_arithmetic_DMUL_H__
...
namespace pf2asm {
  namespace node {
    namespace arithmetic {
      ...
      class DMUL: public cdk::node::Node {
      public:
        inline DMUL(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "DMUL"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processDMUL(this, level);
        }
      };

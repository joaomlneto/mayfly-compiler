// $Id: LT.h,v 1.3 2013/02/12 18:56:17 david Exp $
#ifndef __PF2ASM_NODE_relational_LT_H__
#define __PF2ASM_NODE_relational_LT_H__
...
namespace pf2asm {
  namespace node {
    namespace relational {
      ...
      class LT: public cdk::node::Node {
      public:
        inline LT(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "LT"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processLT(this, level);
        }
      };

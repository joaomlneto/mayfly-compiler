// $Id: UGE.h,v 1.3 2013/02/12 18:56:17 david Exp $
#ifndef __PF2ASM_NODE_relational_UGE_H__
#define __PF2ASM_NODE_relational_UGE_H__
...
namespace pf2asm {
  namespace node {
    namespace relational {
      ...
      class UGE: public cdk::node::Node {
      public:
        inline UGE(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "UGE"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processUGE(this, level);
        }
      };

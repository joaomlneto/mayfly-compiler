// $Id: NE.h,v 1.3 2013/02/12 18:56:17 david Exp $
#ifndef __PF2ASM_NODE_relational_NE_H__
#define __PF2ASM_NODE_relational_NE_H__
...
namespace pf2asm {
  namespace node {
    namespace relational {
      ...
      class NE: public cdk::node::Node {
      public:
        inline NE(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "NE"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processNE(this, level);
        }
      };

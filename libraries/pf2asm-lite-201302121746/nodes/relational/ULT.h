// $Id: ULT.h,v 1.3 2013/02/12 18:56:17 david Exp $
#ifndef __PF2ASM_NODE_relational_ULT_H__
#define __PF2ASM_NODE_relational_ULT_H__
...
namespace pf2asm {
  namespace node {
    namespace relational {
      ...
      class ULT: public cdk::node::Node {
      public:
        inline ULT(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "ULT"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processULT(this, level);
        }
      };

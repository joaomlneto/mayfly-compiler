// $Id: JGE.h,v 1.3 2013/02/12 18:56:16 david Exp $
#ifndef __PF2ASM_NODE_jumps_JGE_H__
#define __PF2ASM_NODE_jumps_JGE_H__
...
namespace pf2asm {
  namespace node {
    namespace jumps {
      ...
      class JGE: public cdk::node::Node {
        std::string _label;

      public:
        inline JGE(int lineno, std::string &label) :
          cdk::node::Node(lineno), _label(label) {
        }
        inline const std::string &label() const {
          return _label;
        }
        inline const char *name() const { return "JGE"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processJGE(this, level);
        }
      };

// $Id: JMP.h,v 1.3 2013/02/12 18:56:16 david Exp $
#ifndef __PF2ASM_NODE_jumps_JMP_H__
#define __PF2ASM_NODE_jumps_JMP_H__
...
namespace pf2asm {
  namespace node {
    namespace jumps {
      ...
      class JMP: public cdk::node::Node {
        std::string _label;

      public:
        inline JMP(int lineno, std::string &label) :
          cdk::node::Node(lineno), _label(label) {
        }
        inline const std::string &label() const {
          return _label;
        }
        inline const char *name() const { return "JMP"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processJMP(this, level);
        }
      };

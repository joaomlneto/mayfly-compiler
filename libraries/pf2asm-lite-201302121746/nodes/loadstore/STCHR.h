// $Id: STCHR.h,v 1.3 2013/02/12 18:56:13 david Exp $
#ifndef __PF2ASM_NODE_loadstore_STCHR_H__
#define __PF2ASM_NODE_loadstore_STCHR_H__
...
namespace pf2asm {
  namespace node {
    namespace loadstore {
      ...
      class STCHR: public cdk::node::Node {
      public:
        inline STCHR(int lineno) :
          cdk::node::Node(lineno) {
        }
        inline const char *name() const { return "STCHR"; }
        inline void accept(SemanticProcessor *sp, int level) {
          sp->processSTCHR(this, level);
        }
      };
